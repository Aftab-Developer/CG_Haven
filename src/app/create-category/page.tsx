...
import {CategorySchema} from "@/validatorModels/createCategorySchema"

export interface CategorysDetails {
  ca_name:string ,
  ca_des:string,
  ca_image:File | null
}
const page = () => {
    const [categoryDetails,setCategoryDetails]=useState<CategorysDetails>({
      ca_name:"" ,
      ca_des:"",
      ca_image:null 
    }); 
  ...
  const submitHandler = async () => {
    try {  
      const res = CategorySchema.safeParse({ca_name: categoryDetails.ca_name,
        ca_des: categoryDetails.ca_des});
      if (!res.success) {
      ...
      const fromData = new FormData(); 
       fromData.append('ca_name',categoryDetails.ca_name) ;
       fromData.append('ca_des',categoryDetails.ca_des) ; 
       if(categoryDetails.ca_image){
         fromData.append('image',categoryDetails.ca_image) ;
       } 
       setCategoryDetails({
        ca_name:"" ,
        ca_des:"",
       ...
       setSelectedFileName("") ;
      const fetchData = await fetch('http://localhost:3000/api/create-category',{
        method:"POST",
        body:fromData
      }); 
...
setLoading(false) ;
                ...
                <div className="w-full sm:w-1/2">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      name="fullName"
                      id="fullName" 
                      value={categoryDetails.ca_name}
                      placeholder="category Name"
                      onChange={(e)=> setCategoryDetails({...categoryDetails,ca_name : e.target.value})}
                    />
                  </div>
                </div>
  ...
  <div className="relative inline-block w-full">
    <input
      ...
      onChange={(e)=>{ setCategoryDetails({...categoryDetails,ca_image : e.target.files? e.target.files[0] : null}); setSelectedFileName(e.target.files? e.target.files[0].name : null)}}
              ...
              <div className="mb-5.5">
                ...
                <div className="relative">
                  ...
                  <textarea
                    ...
                    value={categoryDetails.ca_des}

                    onChange={(e)=> setCategoryDetails({...categoryDetails,ca_des : e.target.value})}
