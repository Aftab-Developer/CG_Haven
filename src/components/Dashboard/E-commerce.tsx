...
//       <div className="mt-4 min-h-screen grid grid-cols-12 gap-4 md:mt-6 md:gap-6 2xl:mt-7.5 2xl:gap-7.5">
//         <ChartOne />  
//         <ChartTwo />
...
const Profile = () => {  
  ...
  const [Bannerfile,setBannerImage ] = useState<string>("");
  const [Profilefile,setProfileImage ] = useState<string>("");
  ...
  async function updationFirst(file: File) {
    try {
      const formData = new FormData(); 
      formData.append("ad_banner_image", file); 
  
      const res = await fetch("http://localhost:3000/api/update-admin", {
        method: "PATCH",
        body: formData,
      });
  
      const resJson = await res.json();
      console.log(resJson.adminDocument[0].ad_banner)
      //   // kindly update the
      //  setBannerImage(resJson.adminDocument[0].ad_banner)
  ...
  async function fixedChanges(file:File) {
     const url = URL.createObjectURL(file) ; 
     setBannerImage(url);   
       await updationFirst(file) ;  
  } 


  async function updationSecond(file: File) {
    try {
      const formData = new FormData(); 
      formData.append("ad_profile_img", file); 
      ...
      const resJson = await res.json();
      console.log(resJson.adminDocument[0].ad_profile_img)
      // kindly update the
      //  setProfileImage(resJson.adminDocument[0].ad_profile_img)
  ...
  async function fixedChangesSecond(file:File) {
    const url = URL.createObjectURL(file) ; 
    setProfileImage(url); 
   await updationSecond(file)
      ...
      <div className="mx-auto max-w-242.5">
        ...
        <div className="overflow-hidden rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
          <div className="relative z-20 h-35 md:h-65">
            <Image
              src={ Bannerfile ? Bannerfile : "/images/cover/cover-01.png"}
              alt="profile cover"
              className="h-full w-full overflow-hidden rounded-tl-sm rounded-tr-sm object-cover object-center"
              width={970}
              height={260}
            ...
            <div className="absolute bottom-1 right-1 z-10 xsm:bottom-4 xsm:right-4">
              ...
              >
                <input
                  ...
                  id="cover"
                  className="sr-only" 
                  onChange={(e) => {e.target.files ? fixedChanges(e.target.files[0]) : null ;
                  }}
                />
                <span>
                </span>
          ...
          <div className="px-4 pb-6 text-center lg:pb-8 xl:pb-11.5">
            <div className="relative z-30 mx-auto -mt-22 h-30 w-full max-w-30 rounded-full  sm:h-44 sm:max-w-44 sm:p-3">
              <div className="relative drop-shadow-2">
                <Image
                  src={Profilefile ? Profilefile : "/images/user/user-06.png"}
                  width={160}
                  height={160}
                ...
                >
                  ...
                  <input
                    ...
                    id="profile"
                    className="sr-only"
                    onChange={(e) => {e.target.files ? fixedChangesSecond(e.target.files[0]) : null ; 
            ...
            <div className="mt-4">
              ...
              <p className="font-medium">3d models creator</p>
